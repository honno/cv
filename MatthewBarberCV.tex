...
\datedsubsection{Data Pipeline Engineer @ \href{https://www.palantir.com/uk/}{Palantir} (via \href{https://www.hexegic.com/}{Hexegic})}{May 2024—}

\listhighlights{
	\item Built and maintained PySpark data pipelines for local government agencies across the country as a subcontractor for Palantir.
	\item Set up monitoring to ensure we know immediately when pipelines are failing, preventing downtime on critical user-facing applications.
	\item Fixed issues reported by customers, often having to quickly learn code, pipelines and user interfaces written by others to resolve problems under tight SLAs.
	\item Monthly on-call rotation to respond to pipeline failures, either fixing them myself or coordinating with other teams.
}
...
%	\item Transitioned a client’s ETL pipeline to the Dagster data orchestration platform for Python, in which I conducted a refactor of their codebase and made deep structural improvements.
	\item Instilled a testing culture in a client's data pipelines team. I encouraged personnel to write tests and CI infrastructure by writing guides, running tutorials and pair programming.
